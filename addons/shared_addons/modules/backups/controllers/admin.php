	...
	public function preset($preset_id = false)
	{
		...
		if(isset($_POST['name']))
		{
			...
			else 
			{
				...
				$preset_info['tables'] = ($this->input->post('tables_select') == 'yes') ? $tables_check : NULL;
		...
		else 
		{
			...
			$data['form_data']['tables_select'] = ($preset_id != false) ? $table_select : 'no';
			
			$data['form_data']['tables_selection'] = $this->presets_m->get_tables();
			$data['form_data']['tables_picked'] = ($preset_id != false) ? explode(', ', $preset->tables) : array();
			$data['form_data']['tables'] = ($preset_id != false) ? $preset->tables : '';
